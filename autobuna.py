...
# --- 1. SISTEMI I LOG-IN ---
def check_password():
    """Kthen True nëse përdoruesi jep fjalëkalimin e saktë."""
    ...
    if not st.session_state["authenticated"]:
        ...
        if st.button("LOG IN"):
            # Këtu mund t'i ndryshosh username dhe password
            if user == "admin" and password == "buna2026":
                st.session_state["authenticated"] = True
                st.rerun()
            else:
                st.error("Përdoruesi ose fjalëkalimi i gabuar!")
        return False
    return True

if check_password():
    # --- 2. LIDHJA ME FIREBASE ---
    ...
    def create_pdf_download_link(html_content, filename="fatura.html"):
        ...
        return f'<a href="data:text/html;base64,{b64}" download="{filename}" style="text-decoration:none;"><button style="background-color:#28a745; color:white; padding:10px; border:none; border-radius:5px; width:100%; cursor:pointer;">💾 SHKARKO FATURËN</button></a>'
    ...
    if menu == "📊 Dashboard":
        ...
        with col_f1:
            viti_zgjedhur = st.selectbox("Viti:", [str(y) for y in range(2024, 2051)], index=2)
        with col_f2:
        ...
        c1.metric("Investuar", f"{hyrje:,.2f} €")
        c2.metric("Shitur", f"{dalje:,.2f} €")
    ...
    # --- 6. GJENDJA E STOKUT ---
        ...
        if stoku:
            for idx, r in enumerate(stoku):
                m, mod = r.get('marka', ''), r.get('modeli', '')
                if search in m or search in mod:
                    with st.expander(f"📌 {m} {mod} - {r.get('ana', 'N/A')}"):
                        st.write(f"Sasia: {r.get('sasia')} | Blerja: {r.get('blerja')}€")
                        if st.button("🗑️ FSHIJ", key=f"d_{idx}"):
                            stoku.pop(idx); save_cloud('stoku', stoku); st.rerun()
    ...
    # --- 7. PRANIM MALLI ---
        ...
        with st.form("forma"):
            ...
            if st.form_submit_button("RUAJ"):
                ...
                save_cloud('stoku', stoku); save_cloud('investimet', investimet); st.success("U ruajt!")

    # --- 8. SHITJE & FATURË ---
    elif menu == "💸 Shitje & Faturë":
        st.title("💸 Shitje dhe Faturë")
        ...
        if stoku:
            ...
            if st.button("KRYEJ SHITJEN"):
                ...
                save_cloud('stoku', stoku); save_cloud('historiku', historiku)
